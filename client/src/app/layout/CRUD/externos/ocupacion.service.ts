...
export class OcupacionService {
   ...
   private urlBase = environment.apiSaeUrl + 'ocupacion';
   ...
   getAll(): Promise<Ocupacion[]> {
      return this.http.get(this.urlBase+'/leer').toPromise().then(response=>response.json() as Ocupacion[]).catch(this.handleError);
   }
   ...
   getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<Ocupacion[]> {
      return this.http.get(this.urlBase+'/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response=>response.json() as Ocupacion[]).catch(this.handleError);
   }
   ...
   get(id: number): Promise<Ocupacion> {
      ...
      return this.http.get(url).toPromise().then(response=>(response.json() as Ocupacion[])[0]).catch(this.handleError);
   }
   ...
   update(entidadTransporte: Ocupacion): Promise<boolean> {
