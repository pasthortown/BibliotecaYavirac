...
@Component({
   selector: 'app-fotoportada',
   templateUrl: './fotoportada.component.html',
   styleUrls: ['./fotoportada.component.scss']
})
...
export class FotoPortadaComponent implements OnInit {
   ...
   entidadSeleccionada: FotoPortada;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   isValid(entidadPorEvaluar: FotoPortada): boolean {
      return true;
   }
   ...
   crearEntidad(): FotoPortada {
      const nuevoFotoPortada = new FotoPortada();
      nuevoFotoPortada.id = 0;
      return nuevoFotoPortada;
   }

   add(entidadNueva: FotoPortada): void {
   ...
   update(entidadParaActualizar: FotoPortada): void {
   ...
   delete(entidadParaBorrar: FotoPortada): void {
