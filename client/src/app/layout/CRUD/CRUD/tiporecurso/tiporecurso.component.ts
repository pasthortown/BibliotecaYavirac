...
@Component({
   ...
   styleUrls: ['./tiporecurso.component.scss']
})

export class TipoRecursoComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoRecurso[];
   entidadSeleccionada: TipoRecurso;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoRecursoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoRecurso): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoRecurso {
      const nuevoTipoRecurso = new TipoRecurso();
      nuevoTipoRecurso.id = 0;
      return nuevoTipoRecurso;
   }

   add(entidadNueva: TipoRecurso): void {
   ...
   update(entidadParaActualizar: TipoRecurso): void {
   ...
   delete(entidadParaBorrar: TipoRecurso): void {
   ...
   onSelect(entidadActual: TipoRecurso): void {
