...
@Component({
   ...
   styleUrls: ['./productora.component.scss']
})

export class ProductoraComponent implements OnInit {

   busy: Promise<any>;
   entidades: Productora[];
   entidadSeleccionada: Productora;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ProductoraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Productora): boolean {
      return true;
   }
   ...
   crearEntidad(): Productora {
      const nuevoProductora = new Productora();
      nuevoProductora.id = 0;
      return nuevoProductora;
   }

   add(entidadNueva: Productora): void {
   ...
   update(entidadParaActualizar: Productora): void {
   ...
   delete(entidadParaBorrar: Productora): void {
   ...
   onSelect(entidadActual: Productora): void {
